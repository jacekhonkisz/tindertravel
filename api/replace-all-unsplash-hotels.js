...
class AllUnsplashReplacer {
  constructor() {
    ...
    this.knownHotelIds = [
      ...
      1377095, 1377096, 1377097, 1377098, 1377099, 1377100, 1377101,
      1377102, 1377103, 1377104, 1377105, 1377106, 1377107, 1377108
    ];
    
    this.currentHotelIdIndex = 0;
  }

  async replaceAllUnsplashHotels() {
    console.log('🚀 REPLACING ALL HOTELS WITH UNSPLASH PHOTOS');
    console.log('='.repeat(70));
    
    try {
      // Get all hotels with Unsplash photos
      ...
      console.log(`🎯 Processing all ${unsplashHotels.length} hotels\n`);

      for (let i = 0; i < unsplashHotels.length; i++) {
        const hotel = unsplashHotels[i];
        console.log(`\n[${i + 1}/${unsplashHotels.length}] Processing: ${hotel.name}`);
  ...
  async findWorkingHotelId() {
    ...
    for (let i = 0; i < this.knownHotelIds.length; i++) {
      const hotelId = this.knownHotelIds[this.currentHotelIdIndex];
      const photos = await this.testHotelPhotos(hotelId);
      if (photos && photos.length > 0) {
        // Move to next ID for variety
        this.currentHotelIdIndex = (this.currentHotelIdIndex + 1) % this.knownHotelIds.length;
        return hotelId;
      }
      this.currentHotelIdIndex = (this.currentHotelIdIndex + 1) % this.knownHotelIds.length;
    }
    ...
    try {
      ...
      const nonUnsplashPhotos = existingPhotos.filter(photo => {
        try {
          ...
          // If parsing fails, check if it's a URL string
          if (typeof photo === 'string' && photo.includes('unsplash')) {
            return false; // Exclude Unsplash URLs
          }
          return true; // Keep other photos
        }
      });
  ...
  generateReport() {
    console.log('\n' + '='.repeat(70));
    console.log('📊 ALL HOTELS REPLACEMENT REPORT');
    console.log('='.repeat(70));
    ...
    if (this.stats.updated > 0) {
      ...
      console.log(`• Processed ${this.stats.updated} hotels successfully`);
      
      console.log('\n💡 NEXT STEPS:');
      console.log('• Build hotel ID database for specific hotels');
      console.log('• Implement hotel name matching logic');
      console.log('• Add photo quality validation');
      console.log('• Consider implementing caching for API responses');
    } else {
      console.log('\n⚠️  NO HOTELS UPDATED');
...
// Run the replacement
const replacer = new AllUnsplashReplacer();
replacer.replaceAllUnsplashHotels().catch(console.error);
