...
// Extended interface for ad-worthy hotel scoring
...
export class AmadeusClient {
  ...
  constructor() {
    ...
    this.tokenCache = new NodeCache({ stdTTL: 1500 });
    
    // Data cache: cache hotel data for 1 hour
    this.dataCache = new NodeCache({ stdTTL: 3600 });

    // Initialize API clients
    this.hotellookClient = new HotellookClient();
  ...
  async getHotelsByCity(cityCode: string, limit: number = 50): Promise<AmadeusHotelOffer[]> {
    const cacheKey = `hotels_${cityCode}_${limit}`;
    const cached = this.dataCache.get<AmadeusHotelOffer[]>(cacheKey);
    if (cached) {
      return cached;
    }
    ...
    try {
      ...
      const hotelsWithOffers: AmadeusHotelOffer[] = [];
  ...
  private getBoutiqueUniqueAmenities(): string[] {
    return [
  ...
  private getVisuallyStunningLocationKeywords(): string[] {
    return [
      // COASTAL PARADISE
   ...
   */
  private calculateVisualAppealScore(hotel: AmadeusHotelOffer, hotelName: string, location: string): number {
    let score = 0;
    ...
    amenityKeywords.forEach(amenity => {
      if (name.includes(amenity)) score += 4;
    });
    ...
    stunningLocations.forEach(location => {
      if (loc.includes(location)) score += 5;
    });
   ...
   */
  private filterAdWorthyHotels(hotels: AmadeusHotelOffer[], location: string = ''): EnhancedAmadeusHotelOffer[] {
    return hotels
      .map(hotel => {
   ...
   */
  private filterAffordableLuxuryHotels(hotels: AmadeusHotelOffer[], location: string = ''): EnhancedAmadeusHotelOffer[] {
  ...
  private filterBoutiqueLuxuryHotels(hotels: AmadeusHotelOffer[], location: string = ''): AmadeusHotelOffer[] {
  ...
  private filterPremiumHotels(hotels: AmadeusHotelOffer[]): AmadeusHotelOffer[] {
    return hotels.filter(hotel => {
      const price = parseFloat(hotel.offers[0]?.price?.total || '0');
      const hotelName = hotel.hotel.name?.toLowerCase() || '';
  ...
  async getHotelContent(hotelId: string): Promise<AmadeusHotelContent | null> {
    const cacheKey = `content_${hotelId}`;
    const cached = this.dataCache.get<AmadeusHotelContent>(cacheKey);
    if (cached) {
      return cached;
    }
  ...
  // Generate unique hotel ID
  private generateHotelId(hotelName: string, city: string): string {
  ...
  private generateRealisticPrice(priceRange: { min: number; max: number; currency: string }): string {
  ...
  private generateRealisticRating(category: string): number {
    const ratingRanges = {
    ...
    const range = ratingRanges[category as keyof typeof ratingRanges] || { min: 4.0, max: 4.5 };
    return Math.round((Math.random() * (range.max - range.min) + range.min) * 10) / 10;
  }

  // Generate real booking URLs
  private generateBookingUrl(hotel: CuratedHotel): string {
  ...
  private getFallbackPhotos(cityName: string): string[] {
  ...
  private async transformToHotelCard(offer: AmadeusHotelOffer, city: SeedCity): Promise<HotelCard | null> {
    try {
      ...
      let hotelCoords: { lat: number; lng: number };
  ...
  private extractAmenityTags(amenities: Array<{ code: string; description: string }>): string[] {
  ...
  private async getHotelPhotos(cityName: string, countryCode: string, hotelName?: string): Promise<string[]> {
    try {
  ...
  private getGlobalVisuallyStunningLocations(): { [continent: string]: string[] } {
    return {
  ...
  private getGlobalInstagramAmenities(): { [category: string]: string[] } {
    return {
   ...
   */
  private calculateGlobalVisualAppealScore(hotel: AmadeusHotelOffer, hotelName: string, location: string): number {
    let score = 0;
    const name = hotelName.toLowerCase();
    const loc = location.toLowerCase();
    ...
    globalLuxuryBrands.forEach(brand => {
      if (name.includes(brand)) score += 1;
    });
   ...
   */
  private getCuratedRealHotelPhotos(cityName: string, countryCode: string, hotelName?: string): string[] {
   ...
   */
  private async getAccurateHotelCoordinates(hotelName: string, city: SeedCity): Promise<{ lat: number; lng: number }> {
    try {
