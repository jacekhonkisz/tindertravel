      ...
      try {
        ...
        if (rawHotels.length > 0) {
          ...
          if (curationResult.cards.length > 0) {
            ...
            console.log(`   🌟 Sample: ${sample.name} - Score: ${(sample as any).glintzScore?.toFixed(2) || 'N/A'} - Photos: ${sample.photos.length}`);
          }
        }
        ...
        console.error(`❌ Error fetching hotels from ${city.name}:`, (error as Error).message);
      }
    }
