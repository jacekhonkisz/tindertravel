...
export class GlobalHotelFetcher {
  ...
  private totalPhotosValidated = 0;
  private totalPhotosRejected = 0;
  ...
  async fetchGlobalHotels(options: {
    ...
    let existingHotels: Set<string> = new Set();
    if (skipExisting) {
    ...
    for (const continent of continents) {
      if (!GLOBAL_DESTINATIONS[continent as keyof typeof GLOBAL_DESTINATIONS]) {
        console.log(`❌ Unknown continent: ${continent}`);
        continue;
      }

      console.log(`\n🌍 Processing ${continent.toUpperCase()}...`);
      const countries = GLOBAL_DESTINATIONS[continent as keyof typeof GLOBAL_DESTINATIONS];
   ...
   */
  private async fetchAndValidatePhotos(hotels: any[], city: string, countryCode: string): Promise<any[]> {
   ...
   */
  private async fetchCityHotels(cityCode: string, maxHotels: number): Promise<RawHotel[]> {
    try {
   ...
   */
  private async getCityCode(cityName: string, countryCode: string): Promise<string | null> {
