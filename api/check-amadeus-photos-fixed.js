...
class AmadeusPhotoChecker {
  ...
  async checkAmadeusConnection() {
    ...
    try {
      ...
      if (tokenResponse.data.access_token) {
        ...
        console.log(`   Client ID: ${this.amadeusClientId}`);
        console.log(`   Base URL: ${this.amadeusBaseUrl}`);
        return true;
      }
    } catch (error) {
  ...
  async checkDatabasePhotos() {
    ...
    try {
      // Get all hotels from database (without tags column)
      ...
      if (searchResponse.data.data && searchResponse.data.data.length > 0) {
        ...
        if (contentResponse.data.data) {
          ...
          if (media.length > 0) {
            ...
            console.log('   ⚠️ No photos available for this hotel');
          }
        }
      }
  ...
  async runFullCheck() {
    ...
    if (amadeusConnected) {
      console.log('\n✅ Amadeus API is working! You can use it to fetch hotel photos.');
    } else {
      console.log('\n⚠️  Amadeus API connection failed. Check your credentials.');
    }
  }
}
