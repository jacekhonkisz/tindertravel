...
class SabreHotelPhotoFetcher {
  ...
  async getRandomHotels(count = 5) {
    ...
    try {
      // Get all hotels first, then randomize
      ...
      // Randomize and select the requested number
  ...
  async getHotelPhotosFromSabre(hotel, accessToken) {
    ...
    try {
      // First, try to search for the hotel using Sabre Hotel Search API
      ...
      // Get photos from the first matching hotel
      ...
      if (photos.length === 0) {
        ...
        // Try descriptive info API as fallback
        console.log('   🔄 Trying descriptive info API...');
  ...
  async fetchPhotosForRandomHotels() {
    ...
    try {
      ...
      for (let i = 0; i < hotels.length; i++) {
        ...
        const photos = await this.getHotelPhotosFromSabre(hotel, accessToken);
...
// Run the photo fetcher
const fetcher = new SabreHotelPhotoFetcher();
