...
class TargetedSerpApiReplacer {
  constructor() {
    ...
    this.targetHotels = [
      ...
      'ALOE Boutique Hotel powered by Anissa Beach',
      'Carmel Beach Hotel',
      'Post Ranch Inn'
    ];
  ...
  async replaceTargetHotels() {
    console.log('🎯 Starting TARGETED SerpApi photo replacement...');
    console.log(`🔑 Using API key: ${this.serpApiKey.substring(0, 10)}...`);
    console.log(`📋 Targeting ${this.targetHotels.length} specific hotels`);
    ...
    try {
      ...
      // Filter to only our target hotels
      const hotels = allHotels.filter(hotel => 
        this.targetHotels.some(target => hotel.name.includes(target))
      );
      
      console.log(`📊 Found ${hotels.length} target hotels to process`);
  ...
  generateSummary() {
    ...
    console.log(`Target Hotels Processed: ${this.stats.total}`);
...
// Run the targeted replacement
async function runTargetedReplacement() {
  const replacer = new TargetedSerpApiReplacer();
  await replacer.replaceTargetHotels();
}

runTargetedReplacement();
