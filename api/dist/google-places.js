...
class GooglePlacesClient {
     ...
     */
    generatePhotoUrl(photoReference, quality = 'high') {
        if (!this.checkAccess()) {
            return '';
        }
        ...
        try {
            ...
            if (response.data.status === 'OK') {
                ...
                if (criteria.maxPrice) {
                    results = results.filter((hotel) => !hotel.price_level || hotel.price_level <= criteria.maxPrice);
                }
                // Limit results
     ...
     */
    async getSpecificHotelPhotos(hotelName, cityName, maxPhotos = 6) {
        if (!this.checkAccess()) {
            return [];
        }
        try {
