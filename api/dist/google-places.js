...
class GooglePlacesClient {
     ...
     */
    async searchHotels(cityName, limit = 10) {
        if (!this.checkAccess()) {
        ...
        try {
            ...
            if (response.data.status === 'OK') {
                const hotels = response.data.results.slice(0, limit).map((hotel) => ({
                    id: hotel.place_id,
                    name: hotel.name,
     ...
     */
    generatePhotoUrl(photoReference, maxWidth = 1600) {
        if (!this.checkAccess()) {
            return '';
        }
     ...
     */
    getHotelPhotoUrls(hotel, maxPhotos = 6) {
     ...
     */
    selectBestPhotos(photos, maxPhotos = 6) {
     ...
     */
    async selectInstagramPhotos(photos, maxPhotos = 6) {
        if (!photos || photos.length === 0)
            return [];
     ...
     */
    async validatePhotoUrl(photoUrl) {
        try {
            ...
            console.warn(`Photo validation failed for ${photoUrl}:`, error.message);
            return false;
        }
    }
    /**
     ...
     */
    async getValidatedInstagramPhotos(photos, maxPhotos = 6) {
        if (!photos || photos.length === 0)
            return [];
     ...
     */
    async getHotelPhotos(cityName, maxPhotos = 6) {
        try {
     ...
     */
    async getSpecificHotelPhotos(hotelName, cityName, maxPhotos = 8) {
        if (!this.checkAccess()) {
            return [];
        }
        ...
        try {
            ...
            for (const query of searchQueries) {
                ...
                try {
                        ...
                        for (const place of response.data.results.slice(0, 3)) {
                            ...
                            if (this.isHotelMatch(place.name, hotelName)) {
                                if (place.photos && place.photos.length > 0) {
                                    ...
                                    const photoUrls = bestPhotos.map((photo) => this.generatePhotoUrl(photo.photo_reference, 1600));
                    ...
                    console.warn(`Failed search query "${query}":`, error.message);
                }
            }
            // Cache the results
     ...
     */
    isHotelMatch(placeName, targetName) {
     ...
     */
    async getPlacePhotos(placeId, maxPhotos) {
        try {
            ...
            if (response.data.status === 'OK' && response.data.result?.photos) {
                return response.data.result.photos.slice(0, maxPhotos).map((photo) => this.generatePhotoUrl(photo.photo_reference, 1600));
            }
            return [];
        }
        catch (error) {
            console.warn(`Failed to get place photos for ${placeId}:`, error.message);
            return [];
        }
    }
    /**
