...
class OptimizedHotelFetcher {
    ...
    async fetch1500QualityHotels() {
        ...
        for (const city of this.PREMIUM_CITIES) {
            ...
            try {
                ...
                const cityHotels = await this.fetchCityHotelsOptimized(city.code, city.name, city.country, existingIds, Math.min(city.expectedHotels, targetRemaining - totalNewHotels));
     ...
     */
    async fetchCityHotelsOptimized(cityCode, cityName, country, existingIds, maxHotels) {
        ...
        for (const hotelOffer of amadeusCityHotels.slice(0, maxHotels * 2)) {
            try {
                ...
                console.log(`   ⚠️  Skipped hotel ${hotelOffer.hotel.hotelId}: ${error.message}`);
                if (error.message.includes('429')) {
     ...
     */
    async fetchAndValidatePhotos(hotels, city, country) {
        ...
        for (const hotel of hotels) {
            try {
                ...
                const photoUrls = googleHotel.photos.map((photo) => photo_validator_1.PhotoValidator.optimizePhotoUrl(photo.url, 1920, 1080));
                // Validate photo quality
                const validatedPhotos = await photo_validator_1.PhotoValidator.validatePhotos(photoUrls);
                ...
                const updatedHotel = {
                    ...hotel,
                    photos: validatedPhotos.map((photo) => photo.url),
...
exports.OptimizedHotelFetcher = OptimizedHotelFetcher;
// CLI execution
