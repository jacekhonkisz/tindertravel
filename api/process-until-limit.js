...
class FullProcessingSystem {
  constructor() {
    ...
    this.stats = {
      ...
      apiCallsUsed: 0,
      maxApiCalls: 250,
      startingApiCalls: 40 // Already used
    };
  }

  async processUntilLimit() {
    console.log('🚀 Processing hotels until 250 API call limit...');
    ...
    try {
      ...
      // Process hotels in batches until we reach the limit
      let batchNumber = 1;
      const batchSize = 10;
      
      for (let i = 0; i < hotels.length && this.stats.apiCallsUsed < (250 - this.stats.startingApiCalls); i += batchSize) {
        const batch = hotels.slice(i, i + batchSize);
        console.log(`\n🔄 Processing batch ${batchNumber} (hotels ${i + 1}-${Math.min(i + batchSize, hotels.length)})`);
        
        for (const hotel of batch) {
          if (this.stats.apiCallsUsed >= (250 - this.stats.startingApiCalls)) {
            console.log(`\n⏹️ Reached API call limit! Stopping processing.`);
            break;
          }
  ...
  generateFinalSummary() {
    ...
    if (totalApiCallsUsed >= 250) {
      ...
      console.log('🏨 All possible hotels have been processed!');
    } else {
...
async function runFullProcessing() {
  const processor = new FullProcessingSystem();
  await processor.processUntilLimit();
}
