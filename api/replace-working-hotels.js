...
class WorkingHotelsReplacer {
  ...
  async replaceWorkingHotels() {
    console.log('🎯 Starting SerpApi photo replacement for WORKING hotels...');
    console.log(`🔑 Using API key: ${this.serpApiKey.substring(0, 10)}...`);
    console.log(`📋 Targeting ${this.workingHotels.length} working hotels`);
    ...
    try {
      ...
      // Filter to only our working hotels
      const hotels = allHotels.filter(hotel => 
        this.workingHotels.some(working => hotel.name.includes(working))
      );
      
      console.log(`📊 Found ${hotels.length} working hotels to process`);
  ...
  generateSummary() {
    console.log('\n📊 WORKING HOTELS REPLACEMENT SUMMARY:');
    console.log('='.repeat(50));
    console.log(`Working Hotels Processed: ${this.stats.total}`);
...
// Run the replacement
async function runWorkingHotelsReplacement() {
  const replacer = new WorkingHotelsReplacer();
  await replacer.replaceWorkingHotels();
}

runWorkingHotelsReplacement();
